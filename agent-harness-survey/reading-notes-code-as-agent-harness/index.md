...
> 会话补充：论文的逻辑主线——骨架句 + 六层递进链。上文的 taxonomy 是"内容地图"，本节是"逻辑骨架"。

### 8.1 骨架句

> **agent 系统的瓶颈在 harness；harness 的统一介质是代码；接口能做什么（§2）、机制如何可靠（§3）、规模怎么扩展（§4）、未来缺什么（§5）——四层递进，最终落点是把 harness 工程变成一门科学（可执行、可检查、有状态、受治理）。**

### 8.2 逻辑主线：六层递进（每层回答一个问题）

| 层 | 追问的问题 | 答案 | 对应章节 |
|----|-----------|------|---------|
| 起点 | agent 系统不可靠，根源在哪？ | 不在模型推理能力，在"模型输出 ↔ 长程行动/持久状态"的连接系统（harness） | §1 |
| 介质 | harness 用什么做统一介质？ | 代码——可执行、可检查、有状态（视角转换：产物 → 运行介质） | §1 |
| 接口 | 介质能做什么？ | 三种角色：想（可执行推理）/ 做（可编程行动）/ 看（可检查环境）——三件事同一个介质 | §2 |
| 机制 | 怎么保证长期可靠？ | 五套机制 + 核心立场：harness 是更大的策略治理系统，代码只是内部介质（规划/记忆/工具/PEV/AHE） | §3 |
| 扩展 | 一个 agent 装不下怎么办？ | 多智能体分工 + 工件中介通信 + 共享代码状态；但大多数系统停在 implicit/file-only（central gap） | §4 |
| 展望 | 还缺什么、往哪去？ | 5 个应用领域 + 7 个开放问题 → 呼吁 harness 工程科学 | §5 |

### 8.3 层间依赖（为什么这条链是完整的）

- 没有"瓶颈在 harness"（起点），就没有找统一介质的意义（介质）
- 没有"代码是统一介质"（介质），§2–§4 就不是在讲同一件事（接口）
- 没有接口与机制的可靠底座（接口+机制），多智能体的"共享状态"无从谈起（扩展）
- 没有 central gap 的暴露（扩展），七个开放问题失去由头（展望）
...
瓶颈在 harness（§1）      ← 问题定义
  ↓ 统一介质 = 代码（§1）  ← 解决方案的介质选择
  ↓ 接口能做什么（§2）     ← 接口层：想/做/看
  ↓ 机制如何可靠（§3）     ← 可靠性层：五机制 + 治理立场
  ↓ 规模怎么扩展（§4）     ← 扩展层：多智能体 + 共享状态 → 暴露 central gap
  ↓ 未来缺什么（§5）       ← 缺口层：7 个开放问题
落点：harness 工程科学
      四属性：可执行 / 可检查 / 有状态 / 受治理
```
...
> **先证明介质（做什么）→ 再解决可靠（如何保证）→ 然后面对规模（如何扩展）→ 最后承认缺口（还缺什么）——102 页反复夯实这条链，终点是 harness 工程的科学化。**
