...
- **5 分钟速读**：先看第 1、2、5、8 节，抓住“为什么 harness 成为瓶颈”。
- **30 分钟系统读**：按 ETCLOVG 七层读第 4 节，重点看每层的设计权衡，而不是记项目名。
- **用于工程设计**：读第 5、6、7 节，把它当作 Agent 系统架构评审清单。

下图是全文的最短路径：模型能力只是起点，真正决定长周期可靠性的，是把模型接入环境、工具、上下文、验证和治理的控制系统。

![Binding-Constraint Thesis](assets/report_diagrams/harness_binding_constraint.svg)

### 图示索引

| 图 | 放置位置 | 解决的阅读问题 |
|---|---|---|
| Binding-Constraint Thesis | 开篇 | 为什么论文说瓶颈从模型转向 harness |
| ETCLOVG 控制闭环 | 第 4 节 | 七层之间不是并列清单，而是控制系统 |
| Task-to-feedback 评估闭环 | 4.6 节 | 为什么评估应从排行榜变成质量控制 |
| Governance 工具调用控制管线 | 4.7 节 | 治理如何贯穿工具调用全过程 |
| Harness 跨层权衡 | 第 5 节 | 成本、质量、速度与能力控制如何互相牵制 |
...
- **Agent 能力不是模型裸能力**，而是模型与 harness 的组合表现。
- **长周期任务的瓶颈常在控制系统**：上下文、工具、沙箱、恢复、验证、治理任何一层薄弱都会拖垮整体。
- **可观测性和治理不是附属功能**，而是生产 Agent 的一等架构层。
- **评估不能只看最终分数**，必须看 trace、轨迹质量、失败归因和回归反馈。
- **Harness 不是越复杂越好**，模型升级后应重新验证哪些控制仍然必要。
...
ETCLOVG 最容易被误读成“七个组件清单”。更准确的读法是：前四层让 Agent 能够行动，后三层让行动可解释、可纠错、可约束。

![ETCLOVG 控制闭环](assets/report_diagrams/etclovg_control_loop.svg)
...
![Task-to-feedback 评估闭环](assets/report_diagrams/task_feedback_lifecycle.svg)

这张图的关键是：评估不是最后给一个分数，而是把执行轨迹转化为诊断、归因和回归测试。
...
![Governance 工具调用控制管线](assets/report_diagrams/governance_pipeline.svg)

这张图有助于把治理从“安全过滤器”改看成运行时控制面：输入、动作、工具返回、人工审批和审计都要纳入同一条链路。
...
这一节是全文最有价值的部分之一：它说明 ETCLOVG 不是七个独立模块，而是一个互相牵制的系统。

![Harness 跨层权衡](assets/report_diagrams/harness_tradeoffs.svg)
