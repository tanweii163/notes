...
### 4.3 Finishing：完整分支收尾流程

`finishing-a-development-branch` 是开发完成后的收尾 skill，四步流程：验证测试 → 检测环境 → 展示 4 选项 → 执行选择。

**起初以为不适用：** 早期直接在 `main` 分支开发，没有 feature 分支可合并。但后来在 `--output` 参数场景中，用 EnterWorktree 创建了 worktree（branch: `worktree-add-output-parameter`），开发完成后完整走了 4 选项流程：

1. 验证 23/23 测试通过
2. 呈现 4 选项：Merge back / Create PR / Keep / Discard
3. 选择 Option 1（合并到 main）
4. fast-forward merge 成功，用 ExitWorktree 清理

**遇到的问题：**
- `git pull` 失败（main 没有 remote tracking）→ 手动 `git merge` 绕过，结果等价
- ExitWorktree 第一次拒绝（worktree 内有未合并 commits）→ merge 成功后再次调用加 `discard_changes: true`
- `git branch -d` 报 "branch not found"（ExitWorktree 已自动删了）→ benign

**核心教训：**
- 先 merge 后 exit worktree 的顺序是硬性要求，反过来 ExitWorktree 会拒绝
- 丢弃操作要求输入 "discard" 精确字符串确认
- Provenance-based cleanup：只清理 superpowers 创建的 worktree，harness 创建的不碰
...
- 收尾：使用 finishing-a-development-branch 完整走完 4 选项流程（验证测试 → 合并 → 清理 worktree）
...
> 这是本文最核心的发现——两个 skill 的设计要求直接冲突。详见第 4 章第 4.1 节。
...
**本次实践：** 详见第 4 章第 4.3 节。早期在 main 分支开发时跳过了，后来在 `--output` 参数场景中完整走了 4 选项流程（包括 worktree 创建、测试验证、合并、清理等全链路）。
...
**与原始 skill 的差距：** 详见第 4 章第 4.3 节的完整分析。核心发现：先 merge 后 exit worktree 的顺序是硬性要求；ExitWorktree 是 harness-owned 的，不能用 `git worktree remove` 绕过。
